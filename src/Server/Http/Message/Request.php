...
class Request extends \Imi\Util\Http\Request implements ServerRequestInterface
{
    /**
     * swoole的http请求对象
    ...
    public function __construct(\swoole_http_request $request)
    {
        ...
        $this->server = array_merge($_SERVER, $request->server);
        $this->cookies = $request->cookie ?? [];
        $this->get = $request->get ?? [];
        $this->post = $request->post ?? [];
        $this->parseParsedBody();
        parent::__construct($this->getRequestUri(), $request->header, $request->rawContent(), strtoupper($this->server['request_method']), $this->getRequestProtocol());
    }
    ...
    {
        $this->server['is_https'] = $isHttps = isset($this->server['https']) && 'on' === $this->server['https'];
        return Uri::makeUri($this->swooleRequest->header['host'], $this->server['path_info'], \http_build_query($this->swooleRequest->get ?? [], null, '&'), null, $isHttps ? 'https' : 'http');
    }
    ...
    {
        list(, $protocol) = explode('/', $this->server['server_protocol'], 2);
        return $protocol;
    }
