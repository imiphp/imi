...
class HttpRoute
{
     ...
     */
    public function addRule(string $url, $callable, RouteAnnotation $annotation = null, array $options = []): void
    {
        if (null === $annotation)
        {
        ...
        $this->rules[$url][spl_object_id($annotation)] = new RouteItem($annotation, $callable, $view, $viewOption, $options);
    }
