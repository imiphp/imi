 ...
 * @property string|string[]|null $method               请求方法；必须是这些请求方法之一才可以被调用；可以是单个方法的字符串，也可以是字符串数组
 * @property string|string[]|null $domain               允许的域名；可以是单个域名的字符串，也可以是字符串数组，支持正则
 * @property string|array|null    $paramsGet            GET参数条件；可以是单个字符串，也可以是数组；取值：id=100 必须包含id，并且值为100、id!=100 或 id<>100 必须包含id，并且值不为100、id 必须包含id参数、!id 必须不包含id参数、"id" => "\d+" 支持正则
 * @property string|array|null    $paramsPost           POST参数条件；可以是单个字符串，也可以是数组；取值：id=100 必须包含id，并且值为100、id!=100 或 id<>100 必须包含id，并且值不为100、id 必须包含id参数、!id 必须不包含id参数、"id" => "\d+" 支持正则
 * @property string|array|null    $paramsBody           JSON、XML参数条件；可以是单个字符串，也可以是数组；取值：id=100 必须包含id，并且值为100、id!=100 或 id<>100 必须包含id，并且值不为100、id 必须包含id参数、!id 必须不包含id参数、"id" => "\d+" 支持正则
 * @property bool                 $paramsBodyMultiLevel JSON、XML参数条件支持以 . 作为分隔符，支持多级参数获取
 * @property string|array|null    $header               请求头条件；可以是单个字符串，也可以是数组；取值：id=100 必须包含id，并且值为100、id!=100 或 id<>100 必须包含id，并且值不为100、id 必须包含id参数、!id 必须不包含id参数、"id" => "\d+" 支持正则
 * @property string|string[]|null $requestMime          请求的mime类型判断；判断请求头中的Content-Type中是否包含这些mime类型之一；支持字符串和字符串数组
