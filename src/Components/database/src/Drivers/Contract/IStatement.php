...
namespace Imi\Db\Drivers\Contract;

use Imi\Db\Drivers\Consts\ParamType;
...
interface IStatement extends \Iterator
{
     ...
     * 绑定一列到一个 PHP 变量.
     */
    public function bindColumn(string|int $column, mixed &$var, int $type = ParamType::PARAM_AUTO, int $maxLength = 0, mixed $driverOptions = null): bool;

    /**
     * 绑定一个参数到指定的变量名.
     */
    public function bindParam(string|int $param, mixed &$var, int $type = ParamType::PARAM_AUTO, int $maxLength = 0, mixed $driverOptions = null): bool;

    /**
     * 把一个值绑定到一个参数.
     */
    public function bindValue(string|int $param, mixed $value, int $type = ParamType::PARAM_AUTO): bool;
