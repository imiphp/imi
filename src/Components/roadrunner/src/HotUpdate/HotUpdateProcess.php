...
class HotUpdateProcess
{
    ...
    public function run(): void
    {
        ...
        $monitor = App::getContainer()->newInstance($this->monitorClass, array_merge($this->defaultPath, $this->includePaths), $this->excludePaths);
        $time = 0;
    ...
    private function initBuildRuntime(): void
    {
        ...
        $cmd = Imi::getImiCmdArray('imi/buildRuntime', [], [
            'changedFilesFile'  => $this->changedFilesFile,
            'confirm'           => true,
            'app-runtime'       => Imi::getCurrentModeRuntimePath('runtime'),
        ]);
        $this->buildRuntimeHandler = proc_open($cmd, self::DESCRIPTORSPEC, $this->buildRuntimePipes);
        if (false === $this->buildRuntimeHandler)
        {
            throw new \RuntimeException(sprintf('Open "%s" failed', implode(' ', $cmd)));
        }
    }
    ...
    private function beginBuildRuntime(array $changedFiles): void
    {
        ...
        try
        {
            ...
            /** @var \Imi\RoadRunner\Server\Http\Server $mainServer */
            $mainServer = ServerManager::getServer('main');
            $mainServer->reload();
        }
        catch (\Throwable $th)
        {
