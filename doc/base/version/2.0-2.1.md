...
* Redis 模型支持[安全删除记录](/v2.1/components/orm/RedisModel.html#%E5%AE%89%E5%85%A8%E5%88%A0%E9%99%A4%E8%AE%B0%E5%BD%95)

* Redis 模型 `hash_object` 模式支持设置字段类型(`json`/`list`/`set`) ([文档](/v2.1/components/orm/RedisModel.html#@Column))

* Redis 模型 `hash_object` 模式在不启用 redis 序列化情况下支持强类型字段
